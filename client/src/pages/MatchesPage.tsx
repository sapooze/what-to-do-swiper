import { useNavigate } from "react-router-dom";
import { useRoomStore } from "../store/useRoomStore";
import { CATEGORY_ICONS, CATEGORY_LABELS } from "../types";

export function MatchesPage() {
  const navigate = useNavigate();
  const { room } = useRoomStore();

  if (!room) {
    navigate("/");
    return null;
  }

  const matches = [...room.matches].reverse();

  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 px-4 py-6">
      <div className="max-w-sm mx-auto space-y-5">
        {/* Header */}
        <div className="text-center">
          <div className="text-5xl mb-2">💑</div>
          <h1 className="text-3xl font-black text-gray-800">Your Matches</h1>
          <p className="text-gray-400 text-sm mt-1">Things you both want to do</p>
        </div>

        {matches.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
            <div className="text-4xl mb-3">🃏</div>
            <p className="text-gray-600 font-medium">No matches yet</p>
            <p className="text-gray-400 text-sm mt-1">Keep swiping!</p>
          </div>
        ) : (
          <ul className="space-y-3">
            {matches.map((match) => (
              <li
                key={match.item.id}
                className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center gap-4"
              >
                <div className="text-3xl">{CATEGORY_ICONS[match.item.category]}</div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-gray-800">{match.item.text}</p>
                  <p className="text-xs text-gray-400">
                    {CATEGORY_LABELS[match.item.category]}
                  </p>
                </div>
                <span className="text-emerald-500 text-xl">✓</span>
              </li>
            ))}
          </ul>
        )}

        {/* Actions */}
        <div className="space-y-3 pb-8">
          <button
            onClick={() => navigate(`/room/${room.code}`)}
            className="w-full py-3.5 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 active:scale-95 transition-all"
          >
            ← Back to Swiping
          </button>
          <button
            onClick={() => navigate("/")}
            className="w-full py-3 text-sm text-gray-400 hover:text-gray-600 transition-colors"
          >
            Start a new room
          </button>
        </div>
      </div>
    </div>
  );
}
